...
function productImage(item, option) {
  ...
  return { primary: local || candidate, fallback: local ? candidate : (candidateIsLocal ? item.reference : '') };
}

function image(url, alt, fallbackUrl = '') {
  if (!url) return '<div class="image-fallback">Imagem<br>indisponível</div>';
  const fallback = fallbackUrl && fallbackUrl !== url
...
function optionsFor(item) {
  return [...(item.options || [])].sort((left, right) => (Number(right.score) || 0) - (Number(left.score) || 0));
}

function visibleEntries() {
  const query = state.query.trim().toLowerCase();
  const products = sortProducts(state.items.filter((item) => {
  ...
  return products.flatMap((item) => optionsFor(item).map((option, index) => ({ item, option, index })));
}
...
function card(entry) {
  ...
  const description = copy[item.category] || 'Uma peça especial, feita para fazer parte da sua rotina.';
  ...
  ].map((tier) => `<div class="tier-price"><span>${tier.label}</span><strong>${money(unitPriceFromWeight(weightInGrams(option), tier.quantity))}</strong><small>por peça</small></div>`).join('');
...
function renderCatalog() {
  ...
  grid.innerHTML = entries.slice(start, start + state.perPage).map(card).join('');
  $('#empty-state').hidden = entries.length > 0;
  renderPagination(entries.length);
  grid.querySelectorAll('.add-to-cart').forEach((button) => button.addEventListener('click', () => openQuantityDialog(button.dataset.productId, button.dataset.optionName)));
}
...
function renderCart() {
  ...
  if ($('#cart-items')) {
    $('#cart-items').innerHTML = summary.lines.length
      ? summary.lines.map((line, index) => `<div class="cart-item"><div><strong>${text(line.label)}</strong><span>${money(line.unitPrice)} por peça · ${line.quantity} un.</span></div><div class="cart-controls"><button type="button" data-cart-action="decrease" data-cart-index="${index}">−</button><span>${line.quantity}</span><button type="button" data-cart-action="increase" data-cart-index="${index}">+</button><button type="button" data-cart-action="remove" data-cart-index="${index}">Remover</button></div></div>`).join('')
      : '<p class="cart-empty">Seu carrinho está vazio.</p>';
  }
  $('#cart-total') && ($('#cart-total').textContent = money(summary.subtotal));
  $('#production-total') && ($('#production-total').textContent = formatDuration(summary.productionMinutes));
  if ($('#page-cart-items')) {
    $('#page-cart-items').innerHTML = summary.lines.length
      ? summary.lines.map((line, index) => `<div class="page-cart-item"><div><strong>${text(line.label)}</strong><span>${line.quantity} un. · ${money(line.unitPrice)} por peça</span></div><div class="cart-controls"><button type="button" data-page-cart="decrease" data-index="${index}">−</button><span>${line.quantity}</span><button type="button" data-page-cart="increase" data-index="${index}">+</button><button type="button" data-page-cart="remove" data-index="${index}">Remover</button></div></div>`).join('')
      : '<p class="cart-empty">Seu carrinho está vazio.</p>';
  }
  $('#page-cart-total') && ($('#page-cart-total').textContent = money(summary.subtotal));
  $('#page-production-total') && ($('#page-production-total').textContent = formatDuration(summary.productionMinutes));
}
...
function updateQuantityPreview() {
  ...
  $('#quantity-production-time').textContent = `Produção estimada: ${formatDuration(lineProductionMinutes(state.pendingItem.item, quantity))}`;
}

function addToCart(productId, optionName, quantity) {
  const existing = state.cart.find((entry) => entry.productId === productId && entry.optionName === optionName);
...
async function loadUserData() {
  ...
  const [addresses, orders] = await Promise.all([apiClient.listAddresses(), apiClient.listOrders()]);
  state.addresses = addresses.addresses || [];
  state.orders = orders.orders || [];
  if (!state.selectedAddressId) {
    state.selectedAddressId = state.addresses.find((address) => address.isDefault)?.id || state.addresses[0]?.id || null;
  }
}
...
function renderCategories() {
  ...
  if (!node) return;
  const categories = [...new Set(state.items.map((item) => item.category).filter(Boolean))].sort((left, right) => left.localeCompare(right, 'pt-BR'));
  node.innerHTML = '<button class="pill active" data-category="all">Tudo</button>' + categories.map((category) => `<button class="pill" data-category="${text(category)}">${text(category)}</button>`).join('');
  node.querySelectorAll('.pill').forEach((button) => button.addEventListener('click', () => {
...
function renderOrders() {
  ...
  container.innerHTML = state.orders.length
    ? state.orders.map((order) => `<article class="info-card"><strong>Pedido ${text(order.id)}</strong><span>Status: ${text(order.status)}</span><span>${new Date(order.createdAt).toLocaleString('pt-BR')}</span><span>Total ${money(order.total)}</span><span>Produção ${formatDuration(Number(order.productionEstimateHours || 0) * 60)}</span></article>`).join('')
    : '<p class="empty-state-inline">Nenhum pedido salvo ainda.</p>';
}

function renderAccountPage() {
  if (currentPage() !== 'account') return;
  $('#backend-badge').textContent = state.backendMode === 'live' ? 'Backend ativo' : 'Modo mock local';
  ...
  $('#account-mode-label').textContent = state.authMode === 'register'
  ...
  if (state.me) {
    ...
    actions.innerHTML = '<button class="button button-secondary" id="account-manage-addresses" type="button">Gerenciar endereços</button><button class="button button-secondary" id="account-logout" type="button">Sair</button>';
...
function fillAddressForm(address = null) {
  ...
  $('#shipping-form-title').textContent = state.editingAddressId ? 'Editar endereço' : 'Novo endereço';
  $('#shipping-save-address').textContent = state.editingAddressId ? 'Salvar alterações' : 'Salvar endereço';
}
...
function renderQuote() {
  ...
  if (!container) return;
  if (!state.quote) {
    container.innerHTML = '<p class="empty-state-inline">Selecione um endereço para recalcular o pedido.</p>';
  ...
  container.innerHTML = state.quote.items.map((item) => `<div class="quote-item"><strong>${text(item.productNameSnapshot)} — ${text(item.optionName)}</strong><span>${item.quantity} un. · ${money(item.unitPrice)} por peça</span><span>${item.unitWeightGrams} g · ${money(item.lineTotal)}</span></div>`).join('');
  $('#shipping-quote-total').textContent = money(state.quote.total);
  $('#shipping-quote-production').textContent = formatDuration(Number(state.quote.productionEstimateHours || 0) * 60);
  $('#shipping-submit').disabled = !state.selectedAddressId || !cartSummary().lines.length;
}
...
function renderShippingPage() {
  ...
  $('#shipping-backend-badge').textContent = state.backendMode === 'live' ? 'Backend ativo' : 'Modo mock local';
  ...
  renderQuote();
  $('#shipping-addresses').querySelectorAll('[data-address-action]').forEach((button) => button.addEventListener('click', async () => {
    ...
    if (action === 'edit') fillAddressForm(state.addresses.find((entry) => entry.id === addressId));
    if (action === 'delete') {
...
async function refreshQuote() {
  if (currentPage() !== 'shipping' || !state.me || !state.selectedAddressId || !cartSummary().lines.length) {
    state.quote = null;
    renderQuote();
    return;
  }
  ...
  try {
    const result = await apiClient.quoteOrder({ addressId: state.selectedAddressId, items: state.cart });
    state.quote = result.quote;
    setStatus('#shipping-quote-status', `Preços confirmados pelo ${state.backendMode === 'live' ? 'backend' : 'mock local'}.`);
  } catch (error) {
    state.quote = null;
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-quantity-delta]').forEach((button) => button.addEventListener('click', () => {
    $('#quantity-input').value = Math.max(1, Number($('#quantity-input').value || 1) + Number(button.dataset.quantityDelta));
    updateQuantityPreview();
  }));
  ...
  $('#quantity-confirm')?.addEventListener('click', () => {
    if (!state.pendingItem) return;
    addToCart(state.pendingItem.item.id, state.pendingItem.option.name, Math.max(1, Number($('#quantity-input').value) || 1));
  ...
  $('#account-page-form')?.addEventListener('submit', async (event) => {
    ...
    const payload = {
      ...
      email: String(form.get('accountEmail') || '').trim().toLowerCase(),
      password: String(form.get('accountPassword') || ''),
      phone: String(form.get('accountPhone') || '').trim()
    };
    setStatus('#account-page-status', state.authMode === 'register' ? 'Criando conta...' : 'Entrando...');
    try {
      const result = state.authMode === 'register' ? await apiClient.register(payload) : await apiClient.login(payload);
      state.me = result.user;
      await loadUserData();
      renderAccountPage();
      setStatus('#account-page-status', state.authMode === 'register' ? 'Conta criada com sucesso.' : 'Login realizado com sucesso.');
  ...
  $('#shipping-form')?.addEventListener('submit', async (event) => {
    ...
    setStatus('#shipping-status', state.editingAddressId ? 'Atualizando endereço...' : 'Salvando endereço...');
    try {
...
async function init() {
  ...
  window.addEventListener('scroll', () => floatingTop?.classList.toggle('is-visible', window.scrollY > window.innerHeight * 0.65), { passive: true });
}
